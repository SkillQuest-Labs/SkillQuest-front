    ...
    <div className={`bg-slate-800/30 rounded-xl border border-slate-600/30 p-6 ${className}`}>
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="text-xs text-slate-400">
          {recentSkills.length} compétence{recentSkills.length > 1 ? 's' : ''}
        </div>
      </div>
      ...
      <div className="space-y-3">
        {recentSkills.map((skill, index) => (
          <div
            key={skill.id || index}
            className="group p-3 rounded-lg bg-slate-700/30 border border-slate-600/20 hover:bg-slate-700/50 hover:border-slate-500/30 transition-all duration-200 cursor-pointer"
          >
            <div className="flex items-start justify-between">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2">
                  <span
                    className={`px-2 py-1 rounded-full text-xs font-medium border ${getStatusColor(skill.status)}`}
                  >
