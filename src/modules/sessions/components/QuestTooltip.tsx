...
export const QuestTooltip = ({ session, children }: QuestTooltipProps) => {
  ...
  return (
    <>
              ...
              <div className="space-y-1.5">
                {session.quests.map((quest, index) => {
                  ...
                  return (
                    <div
                      key={quest.id || index}
                      className={`flex items-start gap-2 text-xs rounded-md px-2 py-1.5 border cursor-pointer transition-colors ${
                        isCompleted
                          ? "text-green-300 bg-green-900/20 border-green-600/30 hover:bg-green-900/30"
                          : "text-slate-300 bg-slate-700/30 border-slate-600/30 hover:bg-slate-700/50"
                      }`}
      ...
      <QuestDetailsDialog isOpen={!!selectedQuest} onClose={closeQuestDetails} quest={selectedQuest} />
    </>
  );
};
